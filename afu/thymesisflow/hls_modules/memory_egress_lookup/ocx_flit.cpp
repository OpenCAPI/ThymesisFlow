...
sel_t OCxFlit::getTLRespState(void)
{
   ...
   //resp_fail =  ((flit.range(511,504) == TL_RD_RESPFAIL) || (flit.range(511,504) == TL_WR_RESPFAIL));
   ...
   resp_rty_only = flit.range(485,485) & (!flit.range(487,487)); //Eat up TLRESP_CODE_PEND | flit.range(486,486);
   resp_rty_pend = (flit.range(485,485) | flit.range(486,486)) & (!flit.range(487,487));

   /*Sel is 001 FAILURE_RTY => RTY: The Opcode is either FAIL or RTY with backoff.
     To reduce logic the  fields are inspected at a more fine grained manner.
   */
   sel.range(0,0) = ((resp_fail & resp_rty_only) | (flit.range(508,508) & flit.range(507,507)));
    ...
    */
   sel.range(2,2) =  flit.range(507,507) | (resp_fail & (!resp_rty_pend));
